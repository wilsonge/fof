...
class Form extends JForm
{
	 ...
	 */
	public static function getInstance(Container $container, $name, $data = null, $options = array(), $replace = true, $xpath = false)
	{
		...
		if (!isset($forms[$name]))
		{
			...
			if (empty($data))
			{
				throw new \InvalidArgumentException(sprintf('FOF: Form::getInstance(name, *%s*)', gettype($data)));
			}

			// Instantiate the form.
			$forms[$name] = new Form($container, $name, $options);
			...
			if (substr(trim($data), 0, 1) == '<')
			{
				...
				{
					throw new \RuntimeException('FOF: Form::getInstance could not load form');
				}
			}
			else
			{
				...
				{
					throw new \RuntimeException('FOF: Form::getInstance could not load file ' . $data . '.xml');
				}
			}
		}
